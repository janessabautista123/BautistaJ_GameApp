
/*
 * Main.eq
 * Generated by Eqela Studio 2.0b7.4
 */

public class Main : SEScene 
{
	SESprite fg;
	SESprite bg;
	SESprite text;
	bool move_right = true;
	bool move_left = false;
	bool move_down = true;
	bool move_up = false;
	int speed = 50;
	double rotation = 0;
	int bounce = 0;
	String my_text;
	
	public void initialize(SEResourceCache rsc) {
		base.initialize(rsc);
		bg = add_sprite_for_color(Color.instance("#99CCFF"), get_scene_width(), get_scene_height());
		bg.move(0, 0);
		fg = add_sprite_for_color(Color.instance("#0000FF"), get_scene_width()*0.1, get_scene_height()*0.1);
		fg.move(100, 100);
		my_text = "Bounce = %d".printf().add(Primitive.for_integer(bounce++)).to_string();
		rsc.prepare_font("myfont", "arial bold color=#202020", 80);
		text = add_sprite_for_text(my_text, "myfont");
		text.move(0, get_scene_height() - text.get_height());
		
	}

	public void update(TimeVal now, double delta) {
		base.update(now, delta);
		rotation+=MathConstant.M_PI/4;
		fg.set_rotation(rotation);
	
		if(move_right) {
			fg.move(fg.get_x() + delta * speed, fg.get_y());
				if(fg.get_x()+fg.get_width() > get_scene_width()) {
					move_right = false;
					move_left = true;
					speed = 50;
					text.set_text("Bounce = %d".printf().add(Primitive.for_integer(bounce++)).to_string());
					}
				speed+=50;
		}
		else {
			fg.move(fg.get_x() - delta * speed, fg.get_y());
			if(fg.get_x() < 0) {
					move_right = true;
					move_left = false;
					speed = 50;
					text.set_text("Bounce = %d".printf().add(Primitive.for_integer(bounce++)).to_string());
			}
			speed+=50;
		}
	
		if(move_down) {
			fg.move(fg.get_x(), fg.get_y() + delta * speed);
				if(fg.get_y()+fg.get_height() > get_scene_height()) {
					move_down = false;
					move_up = true;
					speed = 50;
					text.set_text("Bounce = %d".printf().add(Primitive.for_integer(bounce++)).to_string());
					}
				speed+=50;
		}
		else {
			fg.move(fg.get_x(), fg.get_y() - delta * speed);
			if(fg.get_y() < 0) {
					move_down = true;
					move_up = false;
					speed = 50;
					text.set_text("Bounce = %d".printf().add(Primitive.for_integer(bounce++)).to_string());
			}
			speed +=50;
		}
		
	} //update


	public void set_rotation(double angle) {
		rotation = angle;
	} //rotation

	public double get_rotation() {
		return(rotation);
	}

	public void cleanup() {
		base.cleanup();
		SESprite.remove(fg);
		SESprite.remove(bg);
		SESprite.remove(text);
	}
} //main 

